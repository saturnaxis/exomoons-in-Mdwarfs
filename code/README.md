...
* Run a single initial condition for high-detail investigation.
* Switch between M-dwarf host star types: M0, M2, or M4.
...
* 0.8 to 2.0 Earth masses
* Step size: 0.01 Earth masses

Semi-major axis grid:

* Range set by star type
* Step size: 0.001 AU
* Randomized using a seeded shuffle
...
the script writes outputs inside this directory. Otherwise, it uses the current working directory.
...
Spin state and orbital evolution:
...
If the moon becomes unstable because of the Roche limit, becoming unbound, or escaping the Hill sphere, the file records:
...
* Slices the grid by index.
* Skips completed or unstable runs.
* Restarts incomplete runs.
...
Adjust CPU count, array size, and wall time to match your cluster’s policies.
...
For more sophisticated interior or tidal models, extend the relevant functions and document any changes.
