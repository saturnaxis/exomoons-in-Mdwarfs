...
def calc_Tide_evolution(par_idx):
    ...
    for i in range(1,nsteps):
        ...
        delta_t = [t_i,t_n]
        temp = solve_ivp(deriv_t_Barnes,delta_t,[a_m,e_m,a_p,e_p,O_p],method='RK45',atol=1e-12,rtol=1e-12,args=(kp,tau_p,M_p,R_p,M_sat,M_star))
...
kp = 0.298 #k of host planet = k2p/2 for Hut, kp = k2 for Barnes
...
for m in np.arange(0.8,2.01,0.01):
    for a in np.arange(a_i, a_o,0.001):
        params.append((round(m, 3), round(a, 3)))
...
if len(datafile)>0:
    evaluated = set((round(row[0], 3), round(row[1], 3)) for row in datafile)
